...
#depth= 1 FN= 98 Neoron= [50] Regulization= 0.00010963823939661856
#Learning_Rate = 0.07851426684593897
...
def getmodel(inpudim,sid):
    ...
    #learning_rate=0.001956860213311146
    regu=random.uniform(1,6)
    #regulization=10**(-regu)
    regulization= 0.00010963823939661856
...
def model_training(model_name,model,xtrain,ytrain,xtest,ytest,x_test_new,sid):
    random.seed(sid)
    l_rate=random.uniform(1,6)
    #learning_rate=10**(-l_rate)
    learning_rate= 0.07851426684593897
   ...
   #hist = model.fit(xtrain, ytrain, epochs=epo,batch_size=batch_size,callbacks=cbks,validation_data=(xval,yval),verbose=2)
    ...
    return y_pred_train, y_pred_test, y_pred_test_new
